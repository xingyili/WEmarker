...
def get_G_from_edges(edges, Base):
    edge_dict = dict()
    tmp_G = nx.Graph()
    if Base == True:
        for edge in edges:
        ...
        for edge_key in edge_dict:
            ...
            tmp_G[edge_key.split('_')[0]][edge_key.split('_')
                                          [1]]['weight'] = weight
    else:
        for edge in edges:
            edge_key = str(edge[0]) + '_' + str(edge[1])
            if edge_key not in edge_dict:
                edge_dict[edge_key] = float(edge[2]) #get weights
        for edge_key in edge_dict:
            weight = edge_dict[edge_key]
            tmp_G.add_edge(edge_key.split('_')[0], edge_key.split('_')[1])
            tmp_G[edge_key.split('_')[0]][edge_key.split('_')
...
def train_deepwalk_embedding(walks, iteration=None):
    if iteration is None:
        iteration = 100
    model = Word2Vec(walks, size=200, window=5, min_count=0,
                     sg=1, workers=4, iter=iteration)
    return model


def train_embedding(current_embedding, walks, layer_id, iter=5, info_size=10, base_weight=1):
    training_data = list()
    for walk in walks:
        tmp_walk = list()
        for node in walk:
    ...
    if current_embedding is not None:
        for pos in range(len(current_embedding['index2word'])):
            base_embedding[current_embedding['index2word']
                           [pos]] = current_embedding['base'][pos]
        if layer_id in current_embedding['tran']:
            current_tran = current_embedding['tran'][layer_id]
            current_additional_embedding = dict()
            for pos in range(len(current_embedding['index2word'])):
                current_additional_embedding[current_embedding['index2word']
                                             [pos]] = current_embedding['addition'][layer_id][pos]
            initial_embedding = {
                'base': base_embedding, 'tran': current_tran, 'addition': current_additional_embedding}
        else:
            initial_embedding = {'base': base_embedding,
                                 'tran': None, 'addition': None}
    else:
        initial_embedding = None
    new_model = MNE(training_data, size=200, window=5, min_count=0, sg=1, workers=4, iter=iter,
...
def train_model(network_data):
    ...
    base_G = Random_walk.RWGraph(get_G_from_edges(
        base_network, Base=True), None, 1, 1)
    print('finish building the graph')
    base_G.preprocess_transition_probs()
    base_walks = base_G.simulate_walks(20, 10)
    base_embedding, _, _, index2word = train_embedding(
        None, base_walks, 'Base', 2, 10, 1)
    final_model = dict()
    ...
    for layer_id in network_data:
        if layer_id == 'Base':
            continue
        print('Training model for layer:', layer_id)
        if layer_id not in final_model['addition']:
            final_model['addition'][layer_id] = zeros(
                (len(final_model['index2word']), 10), dtype=REAL)
        tmp_data = network_data[layer_id]
        # start to do the random walk on a layer
        layer_G = Random_walk.RWGraph(
            get_G_from_edges(tmp_data, Base=True), None, 1, 1)
        layer_G.preprocess_transition_probs()
        layer_walks = layer_G.simulate_walks(20, 10)
        tmp_base, tmp_tran, tmp_local, tmp_index2word = train_embedding(
            final_model, layer_walks, layer_id, 5, 10, 0)
        ...
        for tmp_word in tmp_index2word:
            final_model['addition'][layer_id][final_model['index2word'].index(
                tmp_word)] = local_embedding_dict[tmp_word]
    return final_model
...
def save_model(final_model, save_folder_name):
    ...
    for layer_id in final_model['addition']:
        np.save(save_folder_name+'/tran_'+str(layer_id) +
                '.npy', final_model['tran'][layer_id])
        np.save(save_folder_name+'/addition_'+str(layer_id) +
                '.npy', final_model['addition'][layer_id])
...
def load_model(data_folder_name):
    ...
    for layer_id in layer_ids:
        final_model['tran'][layer_id] = np.load(
            data_folder_name+'/tran_'+str(layer_id)+'.npy')
        final_model['addition'][layer_id] = np.load(
            data_folder_name+'/addition_'+str(layer_id)+'.npy')
    return final_model
    ...
    for sentence in sentences:
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                ...
                if pos2 != pos:
                    train_pair(model, model.wv.index2word[word.index], word2.index,
                               alpha, limitation=limitation, base_weight=base_weight)
...
def train_pair(model, word, context_index, alpha, limitation, base_weight, learn_vectors=True, learn_hidden=True,
    ...
    l1 = model.in_base[context_index] + \
        np.dot(model.in_local[context_index], model.in_tran)
    ...
    if learn_vectors:
        ...
        model.in_tran += (1-base_weight) * \
            outer(model.in_local[context_index], neu1e * lock_factor)
        model.in_local[context_index] += (1-base_weight) * \
            dot(neu1e * lock_factor, model.in_tran.T)
...
class MNE(utils.SaveLoad):
    ...
    def __init__(
        ...
        # To warn user if alpha increases
        ...
        if sentences is not None:
            if isinstance(sentences, GeneratorType):
                raise TypeError(
                    "You can't pass a generator as the sentences argument. Try an iterator.")
    ...
    def create_binary_tree(self):
        ...
        for i in range(len(self.wv.vocab) - 1):
            ...
            heapq.heappush(heap, Vocab(count=min1.count + min2.count,
                           index=i + len(self.wv.vocab), left=min1, right=min2))
        ...
        if heap:
            ...
            while stack:
                ...
                if node.index < len(self.wv.vocab):
                    ...
                    points = array(
                        list(points) + [node.index - len(self.wv.vocab)], dtype=uint32)
                    stack.append(
                        (node.left, array(list(codes) + [0], dtype=uint8), points))
                    stack.append(
                        (node.right, array(list(codes) + [1], dtype=uint8), points))

            logger.info(
                "built huffman tree with maximum node depth %i", max_depth)
    ...
    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None):
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            if self.max_vocab_size and len(vocab) > self.max_vocab_size:
                total_words += utils.prune_vocab(vocab,
                                                 min_reduce, trim_rule=trim_rule)
                min_reduce += 1
    ...
    def scale_vocab(self, min_count=None, sample=None, dry_run=False, keep_raw_vocab=False, trim_rule=None, update=False):
        ...
        if not update:
            ...
            for word, v in iteritems(self.raw_vocab):
                if keep_vocab_item(word, v, min_count, trim_rule=trim_rule):
                    ...
                    retain_total += v
                    if not dry_run:
                        self.wv.vocab[word] = Vocab(
                            count=v, index=len(self.wv.index2word))
        ...
        for w in retain_words:
            ...
            if word_probability < 1.0:
                ...
                downsample_total += v
            if not dry_run:
                self.wv.vocab[w].sample_int = int(
                    round(word_probability * 2**32))
    ...
    def finalize_vocab(self, update=False):
        ...
        if self.null_word:
            ...
            # this word is only ever input 鈥?never predicted 鈥?so count, huffman-point, etc doesn't matter
    ...
    def sort_vocab(self):
        ...
        self.wv.index2word.sort(
            key=lambda word: self.wv.vocab[word].count, reverse=True)
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        work, neu1 = inits
        tally = 0
        tally += train_batch(self, sentences, alpha, limitation=self.limitation,
                             work=work, base_weight=self.base_weight)
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        if (self.model_trimmed_post_training):
            raise RuntimeError(
                "Parameters for training were discarded using model_trimmed_post_training method")
        if FAST_VERSION < 0:
            self.neg_labels = []
            if self.negative > 0:
        ...
        if total_words is None and total_examples is None:
            if self.corpus_count:
                ...
                logger.info(
                    "expecting %i sentences, matching count from corpus used for vocabulary survey", total_examples)
            else:
                raise ValueError(
        ...
        def worker_loop():
            ...
            while True:
                ...
                tally, raw_tally = self._do_train_job(
                    sentences, alpha, (work, neu1))
                # report back progress
                progress_queue.put((len(sentences), tally, raw_tally))
                jobs_processed += 1
            ...
            for sent_idx, sentence in enumerate(sentences):
                ...
                if batch_size + sentence_length <= self.batch_words:
                    ...
                    if self.min_alpha < next_alpha:
                        ...
                        next_alpha = self.alpha - \
                            (self.alpha - self.min_alpha) * progress
    ...
    def update_weights(self):
        ...
        for i in range(len(self.wv.syn0), len(self.wv.vocab)):
            ...
            newsyn0[i-len(self.wv.syn0)
                    ] = self.seeded_vector(self.wv.index2word[i] + str(self.seed))
        self.wv.syn0 = vstack([self.wv.syn0, newsyn0])

        if self.hs:
            self.syn1 = vstack(
                [self.syn1, zeros((gained_vocab, self.layer1_size), dtype=REAL)])
        if self.negative:
            self.syn1neg = vstack([self.syn1neg, zeros(
                (gained_vocab, self.layer1_size), dtype=REAL)])
        self.wv.syn0norm = None

        # do not suppress learning for already learned words
        # zeros suppress learning
    ...
    def reset_weights(self):
        ...
        self.in_base = empty(
            (len(self.wv.vocab), self.vector_size), dtype=REAL)
        self.in_local = empty(
            (len(self.wv.vocab), self.small_size), dtype=REAL)
        ...
        if self.initial_embedding is None:
            for i in range(len(self.wv.vocab)):
                ...
                self.wv.syn0[i] = self.seeded_vector(
                    self.wv.index2word[i] + str(self.seed))
                self.in_base[i] = self.seeded_vector(
                    self.wv.index2word[i] + str(self.seed))
                self.in_local[i] = self.seeded_vector2(
                    self.wv.index2word[i] + str(self.seed))
        else:
        ...
        if self.hs:
            self.syn1 = zeros(
                (len(self.wv.vocab), self.layer1_size), dtype=REAL)
        if self.negative:
            self.syn1neg = zeros(
                (len(self.wv.vocab), self.layer1_size), dtype=REAL)
        self.wv.syn0norm = None
            ...
            if binary:
                ...
                for line_no in range(vocab_size):
                    ...
                    if word in self.wv.vocab:
                        ...
                        # lock-factor: 0.0 stops further changes
                        self.syn0_lockf[self.wv.vocab[word].index] = lockf
            else:
                for line_no, line in enumerate(fin):
                    parts = utils.to_unicode(
                        line.rstrip(), encoding=encoding, errors=unicode_errors).split(" ")
    ...
    def estimate_memory(self, vocab_size=None, report=None):
        ...
        if self.hs:
            report['syn1'] = vocab_size * \
                self.layer1_size * dtype(REAL).itemsize
        if self.negative:
    ...
    def save(self, *args, **kwargs):
        ...
        kwargs['ignore'] = kwargs.get(
            'ignore', ['syn0norm', 'table', 'cum_table'])
